...
@dataclass
class TrainerSegmentViewselectionGradientsConfig(ExperimentSegmentConfig):
    """Configuration for training regimen"""

    _target: Type = field(default_factory=lambda: TrainerSegmentViewselectionGradients)
    """Warm-up viewselection"""
    steps_warmup_viewseletion: int = 4000
    """Number of steps between view selection mechanism"""
    steps_per_viewselection: int = 1000
...
class TrainerSegmentViewselectionGradients:
    """Trainer class
    ...
    def __init__(self, config: TrainerSegmentViewselectionGradientsConfig, local_rank: int = 0, world_size: int = 1) -> None:
            ...
            for step in range(self._start_step, self._start_step + num_iterations): #train for 30000 iterations
                ...
                with self.train_lock: #Not relevant since we do not do multiprocessing
                    with TimeWriter(writer, EventName.ITER_TRAIN_TIME, step=step) as train_t:
                        ...
                        # warm-up viewselection
                        if step > self.config.steps_warmup_viewseletion and step % self.config.steps_per_viewselection == 0:
                            training_data = self.pipeline.datamanager.cached_train
