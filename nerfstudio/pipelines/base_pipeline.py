...
@dataclass
class VanillaPipelineSegmentConfig(InstantiateConfig):
    """Configuration for pipeline instantiation"""

    _target: Type = field(default_factory=lambda: VanillaPipelineSegment)
    """target class to instantiate"""
    datamanager: DataManagerConfig = field(default_factory=DataManagerConfig)
    """specifies the datamanager config"""
    model: ModelConfig = field(default_factory=ModelConfig)
...
class VanillaPipeline(Pipeline):
    ...
    def __init__(
        ...
        self.datamanager: DataManager = config.datamanager.setup( #initialized the Datamanager
            device=device, test_mode=test_mode, world_size=world_size, local_rank=local_rank
        )
        # TODO make cleaner
        seed_pts = None
        if ( #we do not go in here
        ...
        self.datamanager.to(device) #Push datamanager to device
        # TODO(ethan): get rid of scene_bounds from the model
        assert self.datamanager.train_dataset is not None, "Missing input dataset"

        self._model = config.model.setup( #initialize model
        ...
        self.model.to(device) #Put model onto device
    ...
    def get_train_loss_dict(self, step: int):
        ...
        Args:
            step: current iteration step to update sampler if using DDP (distributed)
        """
        ray_bundle, batch = self.datamanager.next_train(step) #ray_bundle: world_to_camera coordinates, batch: image to train with 
        model_outputs = self._model(ray_bundle)  # train distributed data parallel model if world_size > 1 #calculate {"rgb": rgb, "depth": depth_im, "accumulation": alpha, "background": background} 
        metrics_dict = self.model.get_metrics_dict(model_outputs, batch)
        loss_dict = self.model.get_loss_dict(model_outputs, batch, metrics_dict) #{"main_loss": main_loss, "scale_reg": scale_reg}
    ...
    def get_training_callbacks(
        self, training_callback_attributes: TrainingCallbackAttributes #This variable holds the pipeline, optimizers, grad_scalers and trainer
    ) -> List[TrainingCallback]:
        """Returns the training callbacks from both the Dataloader and the Model."""
        datamanager_callbacks = self.datamanager.get_training_callbacks(training_callback_attributes) #[]
        model_callbacks = self.model.get_training_callbacks(training_callback_attributes) #[Before training iteration, after training iteration, after training iteration]
...
class VanillaPipelineSegment(Pipeline):
    """The pipeline class for the vanilla nerf setup of multiple cameras for one or a few scenes.
    ...
    def __init__(
        self,
        config: VanillaPipelineSegmentConfig,
        device: str,
        test_mode: Literal["test", "val", "inference"] = "val",
        world_size: int = 1,
        local_rank: int = 0,
        ...
        self._model = config.model.setup( #initialize model
            ...
            grad_scaler=grad_scaler,
            seed_points=seed_pts,
            map_id_color=self.datamanager.map_id_color,
            all_ids=self.datamanager.all_ids
        )
        self.model.to(device) #Put model onto device
