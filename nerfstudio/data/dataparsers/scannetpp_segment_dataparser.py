...
from nerfstudio.data.dataparsers.base_dataparser import DataParser, DataParserConfig, DataparserInstSegmOutputs
...
@dataclass
class ScanNetppSegmentDataParserConfig(DataParserConfig):
    ...
    _target: Type = field(default_factory=lambda: ScanNetppSegment)
    """target class to instantiate"""
    data: Path = Path("scannetpp/410c470782")
    """Directory to the root of the data."""
    data_segment: Path = Path("scannetpp/data_segmentation/410c470782")
    ...
    instance_segm_dir: Path = Path("instance_segmentation/resized_images")
    """Relative path to the segmentation images directory (default: resized_images)"""
    transforms_path: Path = Path("dslr/nerfstudio/transforms.json")
    """Relative path to the transforms.json file"""


@dataclass
class ScanNetppSegment(DataParser):
    """ScanNet++ Segment DatasetParser"""

    config: ScanNetppSegmentDataParserConfig
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        data_inst_seg_dir = self.config.data_segment / self.config.instance_segm_dir
        mask_dir = self.config.data / self.config.masks_dir

        image_filenames = []
        instance_segmentation_filenames = []
        mask_filenames = []
        poses = []
        i_train = []
        i_eval = []
        ...
        for idx, frame in enumerate(frames):
            ...
            filepath_segment = Path(frame["file_path"][:-4] + ".npy")
            fname_inst_segm = data_inst_seg_dir / filepath_segment

            image_filenames.append(fname)
            instance_segmentation_filenames.append(fname_inst_segm)
        ...
        instance_segmentation_filenames = [instance_segmentation_filenames[i] for i in indices]
        ...
        dataparser_outputs = DataparserInstSegmOutputs(
            instance_segm_filenames=instance_segmentation_filenames,
